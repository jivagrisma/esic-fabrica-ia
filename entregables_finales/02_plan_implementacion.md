...
> Punto de partida real: los escenarios 1 y 2 están **implementados en el repo** (Cloud Run + Vertex AI Gemini + Document AI + Firestore); el despliegue a Cloud Run y la verificación E2E se ejecutan al cierre de esta sesión. Este plan describe el camino desde esa base hasta producción con integraciones reales y gobierno de datos.
...
- Inventario de consultas frecuentes (admisiones, financiamiento, empleo/egresados) y clasificación por volumen — valida el supuesto crítico del ROI (consultas/día).
- Curación de la KB institucional: programas vigentes, fechas, requisitos, políticas de financiamiento, bolsa de empleo. Fuente única versionada (hoy `app/kb.py` con datos sintéticos; en producción migraría a Vertex AI Search).
- Definición de tipologías documentales a validar (cédula, transcripts, certificados — ya soportadas en `app/scenario2_docs.py`) y reglas de validación por tipo.
- Muestreo de documentos reales (anonimizados) para calibrar OCR y umbrales de confianza.
- Levantamiento técnico de integraciones: acceso a CRM (p. ej. Dynamics/HubSpot), SIA, Microsoft 365 (Graph API), permisos y firewalls — los contratos ya están documentados en `connectors/*.py`.
...
**Objetivo:** llevar el micro-MVP a calidad piloto con datos reales y usuarios piloto.
...
- Validador: reglas deterministas + análisis semántico Gemini calibradas con el muestreo de Fase 0; veredictos ✅/⚠️/❌ con trazabilidad (`validations`).
...
**Objetivo:** sustituir los conectores simulados por integraciones productivas (los contratos JSON ya existen en `connectors/`).
...
- Microsoft Graph: notificaciones y gestión documental en Microsoft 365 (Teams/correo según el flujo de admisiones).
...
- Escenario 3 (recomendador de programas) en producción sobre datos de empleabilidad, con explicabilidad y auditoría de sesgos por cohorte (diseño ya hecho en el Entregable 3).
...
Todo GCP gestionado (serverless), ya provisionado en el proyecto `esic-fabrica-ia`: Cloud Run (`us-central1`), Vertex AI — Gemini, Document AI (processor OCR multi-región `us`, ENABLED), Firestore (us-central1), Cloud Build/Artifact Registry. Sin servidores que administrar; escala de 0 a N automáticamente.
...
|---|---|---|---|---|
| 1 | Calidad del OCR en documentos escaneados de baja calidad (fotocopias, fotos de celular) | Alto: falsos ⚠️/❌, retrabajo | Media-alta | Umbral de confianza por página (ya implementado en `scenario2_docs.py`); veredicto ⚠️ con alerta específica en vez de rechazo; calibración con muestreo real en Fase 0; re-carga guiada al usuario |
| 2 | Alucinaciones del LLM (respuestas inventadas sobre programas/fechas/financiamiento) | Alto: confianza y reputación | Media | Grounding estricto en KB curada (regla 1 del system prompt); reglas de fuera-de-dominio; derivación a humano; set de regresión de respuestas en CI; RAG con citas en Fase 3 |
...
| 5 | Dependencia de un solo proveedor cloud (Google) | Medio: costo/continuidad a largo plazo | Media | Contenedores estándar (Cloud Run) y contratos de integración portables; LLM detrás de una variable de configuración (`GEMINI_MODEL`); evaluar multi-modelo solo si hay señales de precio o discontinuidad |
...
| 7 | Baja adopción del chatbot por usuarios (prefieren el canal humano) | Medio: no se materializa el ahorro | Media | Canal único visible; respuestas rápidas y útiles desde el día 1; medir y publicar % de resolución; derivación sin fricción con contexto y #ticket |
| 8 | Retraso en accesos a integraciones (CRM/SIA/Microsoft) por permisos internos | Medio: desplaza Fase 2 | Alta | Gestionar accesos desde Fase 0 con TI ESIC; conectores simulados permiten avanzar en paralelo; contratos ya definidos y validados contra esquema |
