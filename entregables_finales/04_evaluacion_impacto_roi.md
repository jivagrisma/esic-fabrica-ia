...
Nota: los 8 min representan el tiempo promedio que hoy consume atender una consulta repetitiva (respuesta + contexto + seguimiento). A calibrar con mediciones de Fase 0.
...
Nota técnica: las tres primeras métricas ya son medibles con el MVP, porque cada interacción y validación se persiste en Firestore desde el día uno (`app/scenario1_chat.py`, `app/scenario2_docs.py`).
...
| Base (70% de horas capturadas) | USD 36.750 | USD 47.900 | ~16 meses (año 2 positivo: el costo recurrente cae al desaparecer la implementación) |
...
- **Escalabilidad sin contratar:** picos de demanda (matrículas, graduaciones) se absorben sin carga operativa adicional (Cloud Run autoscaling).
